...
def column_types():
    ...
    type_list = filter(
        lambda t: not (
            t[0:1] == "_"
            or t.endswith(("type", "TYPE", "Type", "instance"))
            or t.startswith(("Type", "Unicode", "VARBINARY", "Variant"))
        ),
        dir(types),
    )
...
def column_validation(schema_list, connection_name, table_columns=None):
    ...
    for column in schema_list:
        ...
        if column["type"].split("(")[0] not in col_types:
            valid = False
            msg = "Invalid column type for column {}.".format(column["name"])
            break
        try:
        ...
        if column["type"].lower().startswith("string"):
            if "(" not in column["type"] and ")" not in column["type"]:
                ...
                break
        if (
            column["type"].upper() in ["TEXT"]
            and DB_DICT[connection_name].startswith("mysql")
            and str(column["unique"]).upper() == "TRUE"
        ):
            valid = False
            msg = (
                "Unique constraint on TEXT column type is not"
        ...
        if column["default"]:
            if column["type"].upper() in [
                ...
                if isinstance(column["default"], str):
                    valid = False
                    msg = (
                        "The default value entered for column {} is string"
                        " and not of type {}.".format(
            ...
            if column["type"].upper() in ["DATE"]:
                try:
                    ...
                    valid = False
                    msg = (
                        "The format entered for column {} is not correct. "
            ...
            if column["type"].upper() == "BOOLEAN":
                if connection_name == "default":
                    valid = False
                    msg = (
                        "{} datatype for columns is not supported by "
                ...
                if column["default"] not in ["1", "0", "true", "false"]:
                    valid = False
                    msg = (
                        "The default value entered for column {} is not of"
            ...
            if engine == "mysql":
                ...
                if (
                    column["type"].upper()
                    in ["BLOB", "TEXT", "GEOMETRY", "JSON"]
                    and column["default"] is not None
                ):
                    valid = False
                    msg = (
                        "Cannot set a default value for column {} of"
                        " type {}.".format(column["name"], column["type"])
                    )
                    break
    ...
    for table in metadata.sorted_tables:
        if table.name == data["table_name"]:
            valid, msg = column_validation(
                data["columns"], data["app_name"], table.columns
            )
            if valid is False:
                model_data = requests.get(
                    "http://{}:".format(HOST) + PORT + "/" + data["table_name"]
                )
    ...
    for table in metadata.sorted_tables:
        if app_name == extract_database_name(table.info["bind_key"]):
            for column in table.columns:
                if (
                    _type in str(column.type)
                    or str(column.type) in _type.upper()
                ):
                    try:
        ...
        if "relationship" in col:
                ...
                if connection_name == extract_database_name(
                    ...
                    for column in table.columns:
                        if str(column) == related_table + "." + related_field:
                            ...
                            if (
                                col["type"] in str(column.type)
                                or str(column.type) in col["type"].upper()
                            ):
                                valid = True
                                break
                            else:
            ...
            if relation_type.split("-")[0] == "one":
                if str(col["unique"]).upper() != "TRUE":
                    msg = (
                        "The column must have unique constraint for this"
            ...
            if relation_type.split("-")[1] == "one":
                ...
                if column.unique is not True:
                    msg = (
                        "The associated column must have unique constrain"
                ...
                if column.unique is not False:
                    msg = (
                        "The associated column must have not have unique"
                        + " constrain"
                        + "t for this type of relationship, please edit the "
                        + " columns properties."
                    )
